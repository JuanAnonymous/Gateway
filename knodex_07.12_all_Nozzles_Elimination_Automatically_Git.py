...
class knodex:
    ...
    @staticmethod
    def _execute_query(tx: Transaction, cypher_query: str, parameters=None):
        try:
    ...
    # To modify the graph in the Neo4j database(New change:list)
    def remove_nozzle_and_update_flow(self, nozzle_ids: list, component_class_n: str, component_class_m: str):
        for nozzle_id in nozzle_ids:
            ...
            # Nozzle removal and update Relationships (with UNION)
            cypher_query = """
            ...
            RETURN m,o,n
            UNION
            MATCH (o)-[hn:has_nozzle]->(m)
            WHERE m.ComponentClass=$component_class_m AND m.dexpi_id = $nozzle_id
            AND NOT EXISTS ((m)-[:material_flow]->())
            DETACH DELETE m
            RETURN m,o,null as n
            """
    ...
    def _create_relations(tx, triple):
        ...
        try:
            return result.single()[0]
        # Avoid the TypeError, return None instead of raising error
        except TypeError:
            return None
    ...
    # check the implementation and build on that for other instrumentation_functions
...
if __name__ == "__main__":
    # C02
    path = '/Users/.../C02 Process Column (BASF)/C02V03-VER.EX02.xml'
    neo = knodex(path=path, graph_type= "Neo4j", uri="bolt://localhost:...", user="neo4j", password="...")
    neo.build_tree_neo4j()
    print('------------------------------------------------- build_tree_neo4j() implemented successfully! -------------------------------------------------')
    nozzle_ids = ["Nozzle-" + str(i) for i in range(1, 10)]
    neo.remove_nozzle_and_update_flow(nozzle_ids, component_class_n = "PipingNetworkSegment", component_class_m = "Nozzle")
    print('----------------------------------------- remove_nozzle_and_update_flow() implemented successfully! -----------------------------------------')
